...
Persist combat with `bash tools/dm-combat.sh` (start/add-enemy/hp/condition/next-turn/end) so HP and initiative survive resumes.
...
3. Each turn: attack `1d20+bonus` vs AC; on hit roll damage; update HP via `dm-combat.sh hp`.
